...
assert.match(html,/\.conversation-people\{position:absolute;left:calc\(50% - 300px\);top:72%;width:600px/,'three people should use a stable bottom row');
...
assert.match(html,/\.conversation-person-name\{[^}]*font-size:28px/,'desktop names should remain legible after player scaling');
assert.match(html,/@media\(max-width:420px\)\{[\s\S]*\.conversation-person-name\{font-size:34px\}[\s\S]*\.conversation-person-status\{font-size:27px\}/,'small embeds should enlarge person labels before scaling');
...
const atLead=(slot,raw)=>(.10+(slot+raw)*(.78/3));
...
assert.equal(Math.round((.96-.88)*duration),1440,'completed summary should hold before the reset fade');
assert.ok(.22*(.78/3)*duration>=1000,'each moving signal should take at least one second');
...
assert.deepEqual({...context.getConversationRowState(0,purchased)},{status:'Sold',result:'$8',mode:'completed'});
...
assert.deepEqual({...context.getConversationRowState(0,secondLead)},{status:'Sold',result:'$8',mode:'completed'});
