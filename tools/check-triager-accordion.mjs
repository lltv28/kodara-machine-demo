...
assert.match(html,/3 independent conversations/,'header should explicitly explain that three different people are being handled');
...
assert.match(html,/class="conversation-inbox"/,'product shell should keep all three collapsed lead rows visible');
assert.match(html,/conversation-panel-head/,'each lead should have a persistent collapsed identity row');
assert.match(html,/conversation-inbox-copy"><strong>Sandra<\/strong>/,'lead names should remain visible in the vertical stack');
...
assert.match(html,/\.conversation-inbox\{[^}]*display:flex;flex-direction:column/,'lead stack should use a vertical accordion');
assert.match(html,/\.conversation-inbox-row\{[^}]*flex:0 0 50px/,'inactive people should remain visible as compact rows');
assert.match(html,/\.conversation-inbox-row\.is-selected\{flex:1 1 50px/,'selected person should expand into the available conversation space');
