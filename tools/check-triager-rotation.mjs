...
vm.runInNewContext([
  ...
  'setConversationPanel','setConversationSummary','renderConversationCard'
...
function render(progress){
  ...
  const activeY=summaryHeight+gap;
  const geometry={summaryHeight,activeHeight,gap,activeY,nextY:activeY+activeHeight+gap};
  cards.forEach((item,index)=>context.renderConversationCard(item,index,state,geometry));
...
assert.deepEqual(render(.05),[
  ...
  {active:0,summary:1,state:'Handled'}
]);
assert.ok(render(.325)[1].active>.9,'Michael should expand during the first handoff');
...
console.log('triager card rotation valid');
