...
assert.match(page, /<div class="case-support-grid" aria-label="Client stories and results">[\s\S]*?<article class="client-result-card">[\s\S]*?<article class="client-result-card">[\s\S]*?<\/div>\s*<p class="proof-note"/, 'Dr. Vora and Ashley must share the same grid as the video stories');
