...
assert.match(confirmation, /Real Results: Behind-the-Scenes Numbers From 2 Of Our Clients/, 'Confirmation case study must use the approved results-first headline');
...
assert.match(confirmation, /<wistia-player media-id="wn2g27b0vb" aspect="1\.7777777777777777" aria-label="Kodara full presentation replay">/, 'Confirmation page must replay the approved Wistia presentation');
assert.match(confirmation, /https:\/\/fast\.wistia\.com\/embed\/wn2g27b0vb\.js/, 'Confirmation page must load the approved Wistia presentation source');
assert.equal((confirmation.match(/id="vidalytics_embed_[^"]+"/g) ?? []).length, 1, 'Confirmation page needs only the call-confirmation Vidalytics video');
assert.match(confirmation, /<h2 id="reminder-title">We're looking forward to speak with you!<\/h2>/, 'Confirmation page must use the approved closing headline');
assert.match(confirmation, /We started Kodara to help doctors, clinic owners, and wellness experts reach as many people as possible with their expertise\. We appreciate your trust\./, 'Confirmation page must use the approved closing message');
...
assert.equal(confirmationPlayers.length, 16, 'Confirmation page needs one case study, four testimonials, ten FAQ videos, and one presentation replay');
...
assert.doesNotMatch(page, /#hero-region-subheadline\{[^}]*min-block-size/, 'Static hero guarantee must not reserve blank height');
assert.match(page, /\.hero-vsl-stage\{[^}]*margin-top:var\(--space-4\)/, 'Desktop VSL must use the compact gap below the guarantee');
assert.match(page, /@media\(max-width:720px\)[\s\S]*?\.hero-region-eyebrow\{[^}]*min-block-size:2\.8em[\s\S]*?#hero-region-subheadline\{[^}]*font-size:var\(--type-hero-subheadline\)/, 'Mobile hero must reserve only the personalized availability line');
assert.match(page, /@media\(max-width:720px\)[\s\S]*?\.hero-vsl-stage\{margin-top:var\(--space-3\)/, 'Mobile VSL must use the tighter gap below the guarantee');
...
assert.match(searchChartModule, /rootMargin: '180px 0px'/, 'Search chart must begin shortly before it enters the viewport');
assert.match(searchChartModule, /LANDING_SEQUENCE_DURATION_MS = 16000/, 'Search chart must use the calmer 16-second landing-page sequence');
assert.match(searchChartModule, /scheduleCycle/, 'Search chart must repeat while it remains near the viewport');
assert.match(searchChartModule, /root\.dataset\.chartState = 'paused'/, 'Search chart must stop when it leaves the playback range');
...
assert.match(page, /<article class="homepage-stat homepage-stat--compact">\s*<strong>105,000\+<\/strong>\s*<span>Leads &amp; consultations generated by our team and used to train the Kodara AI model<\/span>/, 'The lead-and-consultation stat must retain its compact display treatment');
